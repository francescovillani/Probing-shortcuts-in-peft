...
class ExperimentTracker:
    ...
    def __init__(
        ...
        debug_samples: Optional[Dict[str, List[Dict[str, Any]]]] = None,
    ):
    ...
    def add_epoch_metrics(
        ...
        for dataset_name, results in validation_results.items():
            ...
            if current_metric > best_so_far:
                ...
                if (
                    dataset_name
                    == list(self.metrics["config"]["validation_datasets"].keys())[0]
                ):
    ...
    def save_results(self):
        ...
        summary = {
            ...
            "avg_epoch_time": self.metrics["training"]["total_training_time"]
            / len(self.metrics["training"]["epochs"]),
...
class TrainingRunner:
    ...
    def __init__(self, config: TrainingConfig, logger: logging.Logger):
        ...
        if self.is_evaluation_only:
            self.logger.info(
                "Running in evaluation-only mode (no training dataset provided)"
            )
    ...
    def setup_model_and_data(self):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(
            self.config.model.base_model, use_fast=False
        )
        ...
        # Prepare datasets
        self.train_loader, self.val_loaders, debug_samples, _ = (
            self.dataset_service.prepare_datasets(
        ...
        if (
            ...
            and self.config.train_dataset.poisoning
            and self.config.train_dataset.poisoning.enabled
        ):
            poison_config_dict = self.config.train_dataset.poisoning.model_dump()
            self.logger.info(
                f"Training dataset poisoned with config: {poison_config_dict}"
            )
            if wandb.run is not None:
                wandb.config.update({"train_poisoning_config": poison_config_dict})
            self.tracker.metrics["config"][
                "train_poisoning_config"
            ] = poison_config_dict
        ...
        for val_name, val_config in self.config.validation_datasets.items():
            if val_config.poisoning and val_config.poisoning.enabled:
                ...
                self.logger.info(
                    f"Validation dataset '{val_name}' poisoned with config: {poison_config_dict}"
                )
                if wandb.run is not None:
                    wandb.config.update(
                        {f"{val_name}_poisoning_config": poison_config_dict}
                    )
                self.tracker.metrics["config"][
                    f"{val_name}_poisoning_config"
                ] = poison_config_dict

        # Log differential privacy configuration if enabled (only relevant for training)
        if (
            not self.is_evaluation_only
            and self.config.differential_privacy
            and self.config.differential_privacy.enabled
        ):
            dp_config_dict = self.config.differential_privacy.model_dump()
            self.logger.info(
                f"Differential privacy enabled with config: {dp_config_dict}"
            )
            if wandb.run is not None:
        ...
        if self.is_evaluation_only:
            if not self.config.model.checkpoints_dir:
                raise ValueError(
                    "For evaluation-only mode, model.checkpoints_dir must be specified"
                )

            # Get checkpoint paths and use the latest one if multiple are available
            checkpoint_paths = self.model_service.get_checkpoint_paths(
                self.config.model.checkpoints_dir
            )
            if not checkpoint_paths:
                raise ValueError(
                    f"No checkpoints found in {self.config.model.checkpoints_dir}"
                )
            ...
            self.model = self.model_service.create_model(
                config=self.config.model, num_labels=self.config.num_labels
            )
            self.model.train()
        ...
        if hasattr(self.model, "base_model") and hasattr(
            ...
            if (
                hasattr(self.model.base_model.config, "pad_token_id")
                and self.model.base_model.config.pad_token_id is None
            ):
                self.model.base_model.config.pad_token_id = self.tokenizer.pad_token_id
                self.logger.info(
                    f"Set base model pad_token_id to {self.tokenizer.pad_token_id}"
                )

        # Skip optimization setup for evaluation-only mode
        if self.is_evaluation_only:
            self.logger.info(
                "Skipping optimizer and scheduler setup for evaluation-only mode"
            )
            return
        ...
        if (
            ...
            self.logger.info(
                f"Differential privacy enabled with noise_multiplier={dp_config.noise_multiplier}, max_grad_norm={dp_config.max_grad_norm}"
            )
            ...
            self.config.differential_privacy
            and self.config.differential_privacy.enabled
            and not OPACUS_AVAILABLE
        ):
            self.logger.warning(
                "Differential privacy requested but Opacus is not available. Install opacus>=1.4.0 to enable this feature."
            )
    ...
    def run_training(self):
        ...
        for epoch in range(self.config.epochs):
            ...
            privacy_budget = None
            if self.privacy_engine is not None and hasattr(
                self.privacy_engine, "get_privacy_spent"
            ):
                try:
                    epsilon, delta = self.privacy_engine.get_privacy_spent()
                    self.logger.info(
                        f"Epoch {epoch + 1} - Privacy budget: ε={epsilon:.2f}, δ={delta}"
                    )
    ...
    def run_validation(self):
        ...
        for dataset_name, val_loader in self.val_loaders.items():
            ...
            compute_confidence = (
                self.config.compute_confidence_metrics
                and val_config.poisoning
                and val_config.poisoning.enabled
            )
            target_label = (
                val_config.poisoning.target_label if compute_confidence else None
            )

            # Check if we should compute hidden similarities
            compute_similarities = (
                self.config.compute_hidden_similarities
                and val_config.poisoning
                and val_config.poisoning.enabled
            )
            ...
            if wandb.run is not None and "confidence_metrics" in results:
                ...
                wandb.log(
                    {
                        f"val/{dataset_name}/target_confidence_mean": confidence_results[
                            "target_confidence"
                        ][
                            "mean"
                        ],
                        f"val/{dataset_name}/target_confidence_std": confidence_results[
                            "target_confidence"
                        ]["std"],
                        f"val/{dataset_name}/logit_diff_mean": confidence_results[
                            "logit_differences"
                        ]["mean"],
                        f"val/{dataset_name}/logit_diff_std": confidence_results[
                            "logit_differences"
                        ]["std"],
                        f"val/{dataset_name}/target_prediction_rate": confidence_results[
                            "prediction_stats"
                        ][
            ...
            if wandb.run is not None and "hidden_similarities" in results:
                ...
                if (
                    "hidden_similarities" in similarity_results
                ):  # Check for valid results (not error)
                    sim_stats = similarity_results["hidden_similarities"]
                    wandb.log(
                        {
                            f"val/{dataset_name}/hidden_similarity_mean": sim_stats[
                                "mean"
                            ],
                            f"val/{dataset_name}/hidden_similarity_std": sim_stats[
                                "std"
                            ],
                            f"val/{dataset_name}/hidden_similarity_median": sim_stats[
                                "median"
                            ],
                            f"val/{dataset_name}/hidden_similarity_samples": sim_stats[
                                "samples_processed"
                            ],
                        }
                    )
            ...
            if wandb.run is not None:
                wandb.log(
                    {
                        f"val/{dataset_name}/{k}": v
                        for k, v in results.items()
                        if not isinstance(v, dict)
                    }
                )  # Skip nested dicts for wandb logging
...
def start_training(config: TrainingConfig):
    ...
    if config.wandb.enabled:
        try:
            wandb.init(
                project=config.wandb.project, config=config.model_dump(), reinit=True
            )
        except Exception as e:
    ...
    try:
        ...
        logger.info(
            "Training completed. Results saved to: %s", runner.output_dir / "results"
        )
        return results
    except Exception as e:
